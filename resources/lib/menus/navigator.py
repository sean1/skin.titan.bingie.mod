...
add_item, add_items, add_dir = ku.add_item, ku.add_items, ku.add_dir
...
class Navigator:
	...
	def search(self):
		...
		self._add_item({'mode': 'get_search_term', 'mediatype': 'movie',                                            'name': mov_str   }, 'search_movie.png' , s_n_ins, False)
		self._add_item({'mode': 'get_search_term', 'mediatype': 'tv_show',                                          'name': tv_str    }, 'search_tv.png'    , s_n_ins, False)
		self._add_item({'mode': 'get_search_term', 'search_type': 'people',                                        'name': people_str}, 'search_people.png', s_n_ins, False)
		self._add_item({'mode': 'get_search_term', 'search_type': 'tmdb_collections', 'mediatype': 'movie',         'name': coll_str  }, 'search_tmdb.png'  , s_n_ins, False)
		self._end_directory()

	def settings(self):
		manager_str, changelog_str, short_str = ls(32513), ls(32508), ls(32514)
		log_utils, views_str, clean_str, ms_str = ls(32777), ls(32510), ls(32512), ls(32455)
		settings_str, changelog_log_viewer_str = ls(32247), '%s & %s' % (changelog_str, log_utils)
		shortcut_manager_str = '%s %s' % (short_str, manager_str)
		external_manager_str = '%s %s' % (ls(32118), manager_str)
		n_ins = _in_str % (settings_str.upper(), '')
		self._add_item({'mode': 'myservices',                    'name': ms_str                  }, 'settings.png', n_ins, False)
		self._add_item({'mode': 'toggle_provider',               'name': external_manager_str   }, 'settings.png', n_ins, False)
	...
	def clear_info(self):
		cache_str, clca_str, clean_str, all_str = ls(32524), ls(32497), ls(32526), ls(32525)
		...
		clear_rd_str = clca_str % ls(32054)
	...
	def log_utils(self):
		pov_vstr = ku.get_addoninfo('version')
		kl_loc, mt_str = 'special://logpath/kodi.log', ku.addon_path + 'changelog.txt'
