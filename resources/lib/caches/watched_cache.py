...
from modules.utils import LIST_WORKERS, adjust_premiered_date, get_datetime, paginate_list, sort_for_article, TaskPool
...
def get_last_played_value(database_type=None):
	return datetime.now().strftime('%Y-%m-%d %H:%M:%S')

def get_resumetime(bookmarks, tmdb_id, season='', episode=''):
	try: resume_point, curr_time, _ = detect_bookmark(bookmarks, tmdb_id, season, episode)
...
def get_next_episodes(watched_indicators):
	...
	return [
		...
		for i in dbcur.execute(command, ('episode',)).fetchall() if int(i[0]) not in dropped_info
	]
...
def get_watched_info_movie(watched_indicators):
	...
	try:
		...
		dbcon = _database_connect(get_database(watched_indicators))
		dbcur = set_PRAGMAS(dbcon)
		for i in dbcur.execute(command, ('movie',)).fetchall(): info[i[0]] = i
	except: pass
...
def get_watched_info_tv(watched_indicators):
	...
	try:
		...
		dbcon = _database_connect(get_database(watched_indicators))
		dbcur = set_PRAGMAS(dbcon)
		for i in dbcur.execute(command, ('episode',)).fetchall():
...
def get_dropped_info_tv(watched_indicators):
	hidden_data = set()
	try: hidden_data.update(get_hidden_items('dropped'))
	except: pass
	return hidden_data
...
def get_in_progress_tvshows(watched_info, mediatype, page_no):
	...
	for i in TaskPool(LIST_WORKERS).tasks(_process, prelim_data, Thread): i.join()
