...
def clear_streams():
	...
	if not dialog.yesno('BINGIE Lite', f"{dbfile.name}[CR][CR]Are you sure?"): return
	minimum_uses = 30
	...
	with database.connect(str(dbfile)) as dbcon:
		...
		if not dialog.yesno('BINGIE Lite', '[CR][CR]Delete %d Items?' % expired): return
...
def clear_thumbnails():
	...
	with database.connect(str(dbfile)) as dbcon:
		...
		if expired and not dialog.yesno('BINGIE Lite', '[CR][CR]Delete %d Items?' % expired): return
		progress_dialog = xbmcgui.DialogProgress()
