...
class Discover:
	def __init__(self, params):
		...
		self.window_id = 'pov_lite_%s_discover_params' % self.mediatype.upper() if self.mediatype else ''
...
def help(): return (
"""
[COLOR dodgerblue][B]BINGIE Lite Discover[/B][/COLOR]

BINGIE Lite Discover allows you to browse and/or export lists that you make using the filter values you provide.
Only the filters you wish to use need to be assigned a value; the rest can be left blank. Once you've set your
desired filters, select 'Browse Results' to view the list or 'Menu Export' to place it in one of the BINGIE Lite Main
Menus (Root, Movies or TV Shows). Discover queries are not saved as history. Below are two examples:
...
- Select "[B]Browse Results[/B]" to immediately see the results or "[B]Menu Export[/B]" to export the
  list to BINGIE Lite Root Menu or BINGIE Lite Movies Menu etc
