...
from modules.utils import LIST_WORKERS, jsondate_to_datetime, subtract_dates, TaskPool
# from modules.kodi_utils import logger

EXPIRES_2_DAYS, EXPIRES_4_DAYS, EXPIRES_7_DAYS, EXPIRES_14_DAYS, EXPIRES_182_DAYS = 2, 4, 7, 14, 182
backup_resolutions = {'logo': 'w300', 'poster': 'w342', 'fanart': 'w1280', 'still': 'w300', 'profile': 'w342'}
finished_show_check, empty_value_check = ('Ended', 'Canceled'), ('', 'None', None)
alt_titles_test, trailers_test = ('US', 'GB', 'UK', ''), ('Trailer', 'Teaser')
trailer_name_reject = ('vertical', 'portrait', '9:16', '9x16', 'short video', 'short trailer', 'short teaser', 'youtube short', 'social cut', 'social spot', '" video')
writer_credits = ('Author', 'Writer', 'Screenplay', 'Characters')
tmdb_image_base, resized_tmdb_image = tmdb_api.tmdb_image_base, tmdb_api.resized_tmdb_image
...
def art_infodict(meta, art_provider, meta_user_info, extra_art=None):
	...
	poster = resized_tmdb_image(meta_get(poster_main) or meta_get(poster_backup), 'w342') or poster_empty
	fanart = meta_get(fanart_main) or meta_get(fanart_backup) or fanart_empty
	clearlogo = resized_tmdb_image(meta_get('clearlogo'), 'w300') or ''
	banner, clearart, landscape, discart = '', '', resized_tmdb_image(fanart, 'w780'), ''
	...
	art = {
		'poster': poster, 'fanart': fanart, 'icon': poster, 'thumb': landscape, 'clearlogo': clearlogo,
	...
	if extra_art.get('season_poster') or False:
		season_poster = resized_tmdb_image(extra_art.get('season_poster'), 'w342') or poster
		art.update(dict.fromkeys(('season.poster', 'poster'), season_poster))
	if extra_art.get('thumb') or False:
		thumb = resized_tmdb_image(extra_art.get('thumb'), 'w300')
		art.update(dict.fromkeys(('thumb', 'icon', 'landscape', 'tvshow.landscape'), thumb))
	return art
...
def main_actors(cast, limit=3):
	return ', '.join(item.get('name', '') for item in cast[:limit] if item.get('name'))

def resized_cast(cast, resolution='w342'):
	result = []
	for item in cast or ():
		...
		item['thumbnail'] = resized_tmdb_image(item.get('thumbnail'), resolution) or ''
		result.append(item)
	return result
...
def _prepare_tvshow_meta(meta, current_date):
	...
	return _adjust_total_aired_eps(_set_tvshow_year_range(_set_best_trailer(meta)), current_date)

def _tvshow_meta(id_type, media_id, user_info, current_date, metacache, claim_owner=None):
	meta, metacache_set = None, metacache.set
	try:
		if id_type == 'tmdb_id':
...
#		meta = build_tvshow_meta(data, user_info)
		...
		meta = _set_tvshow_year_range(meta)
		metacache_set('tvshow', id_type, meta, tvshow_expiry(current_date, meta))
	except: pass
	return _adjust_total_aired_eps(meta, current_date)

def _set_tvshow_year_range(meta):
	try:
		if meta.get('status') not in finished_show_check: return meta
		start_year = str(meta.get('year') or '')
		last_air_date = meta.get('extra_info', {}).get('last_episode_to_air', {}).get('air_date', '')
		end_year = next(iter(last_air_date.split('-')), '')
		if start_year: meta['year_range'] = '%s–%s' % (start_year, end_year) if end_year and end_year != start_year else start_year
	except: pass
	return meta
...
def all_episodes_meta(meta, user_info, Thread):
	...
	try:
		...
		for i in TaskPool(LIST_WORKERS).tasks(_get_tmdb_episodes, seasons, Thread): i.join()
	except: pass
	return data
...
def select_trailer(videos):
	...
	if trailers: candidates = trailers
	else: candidates = [video for video in candidates if any(marker in _trailer_name(video) for marker in ('trailer', 'teaser'))]
	if not candidates: return ''
	from modules.trailers import plugin_url
	return plugin_url(max(candidates, key=_trailer_rank)['key'])

def _set_best_trailer(meta):
	if 'all_trailers' in meta: meta['trailer'] = select_trailer(meta.get('all_trailers'))
	return meta
...
def _trailer_rank(video):
	...
	if 'teaser' in name: name_rank = 0
	elif name == 'official trailer': name_rank = 5
	elif name.startswith('official trailer'): name_rank = 4
	elif 'official trailer' in name: name_rank = 3
	elif 'final trailer' in name: name_rank = 2
	elif 'trailer' in name: name_rank = 1
	else: name_rank = 0
	try: size = int(video.get('size') or 0)
	except: size = 0
	return name_rank, bool(video.get('official')), size, video.get('published_at') or ''
