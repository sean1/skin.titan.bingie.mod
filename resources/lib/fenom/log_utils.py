...
import inspect
from fenom.control import transPath, lang, joinPath, existsPath, addonInfo
...
#			addon = 'script.module.fenomscrapers'
			...
			filename = filename.replace(transPath(addonInfo('path')), '')
...
def view_LogFile(name):
	try:
		...
		with open(log_file, 'r', encoding='utf-8', errors='ignore') as f: text = f.read()
		heading = '[B]%s -  LogFile[/B]' % name
		return kodi_utils.show_text(heading, text=text)
	except:
		error()
